...
export default function Navbar(){
  ...
  return(
    <nav style={{position:'fixed',width:'100%',zIndex:50,transition:'all 0.4s'}} className={scrolled?'nav-glass':''}>
      <div className="container flex items-center justify-between" style={{padding:'1rem 2rem'}}>
        <Link to="hero" smooth className="text-gradient flex items-center gap-2 cursor-pointer"
          style={{fontSize:'1.4rem',fontWeight:800,letterSpacing:'-0.04em',fontFamily:"'Outfit',sans-serif",display:'flex',alignItems:'center',gap:'0.5rem'}}>
      ...
      <AnimatePresence>
        {open&&(
          <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}}
            style={{background:'rgba(3,7,18,0.97)',borderBottom:'1px solid var(--border)',overflow:'hidden',padding:'1.5rem 2rem',display:'flex',flexDirection:'column',gap:'1.25rem'}}>
            {links.map(l=>(
      ...
      <style>{`@media(max-width:768px){.desktop-flex{display:none!important;}.mobile-only{display:flex!important;}}`}</style>
    </nav>
  );
}
